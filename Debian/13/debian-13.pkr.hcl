# Debian 13 (Trixie)
# Packer build template Debian 13 (Trixie) on Proxmox
...
build {
  sources = ["proxmox-iso.Debian-13-Trixie"]
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "sudo apt autoremove",
      "sudo hostnamectl set-hostname debian-13-template",
      "sudo sed -i 's/^127\\.0\\.1\\.1\\s\\+debian\\b/127.0.1.1 debian-13-template/g' /etc/hosts",
